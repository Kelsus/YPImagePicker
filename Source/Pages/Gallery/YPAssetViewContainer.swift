...
final class YPAssetViewContainer: UIView {
    ...
    public func execPhotoZoom(animated: Bool = true, forceFit: Bool = false) {
        let z = zoomableView.zoomScale
        shouldCropToSquare = (z >= 1 && z < zoomableView.squaredZoomScale)
        zoomableView.fitImage(shouldCropToSquare || forceFit, animated: animated)
    }
    ...
    public func updateSquareCropButtonState() {
        guard !isMultipleSelectionEnabled else {
            ...
            squareCropButton.isHidden = YPImagePickerConfiguration.shared.library.onlySquare
            return
        }
